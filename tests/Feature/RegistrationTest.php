...
class RegistrationTest extends TestCase
{
    ...
    {
        factory(User::class)->create(['name' => 'JonathanGarcia']);
        $this->post(
            route('register'),
            $this->userValidData(['name' => 'JonathanGarcia'])
        )->assertSessionHasErrors('name');
    }
    ...
    function the_name_may_only_contain_letters_and_numbers()
    {
        $this->post(
            route('register'),
            $this->userValidData(['name' => 'Jonathan Garcia'])
        )->assertSessionHasErrors('name');
    ...
    function the_first_name_may_only_contain_letters()
    {
        $this->post(
            route('register'),
            $this->userValidData(['first_name' => 'Jonathan2'])
        )->assertSessionHasErrors('first_name');

        $this->post(
            route('register'),
            $this->userValidData(['first_name' => 'Jonathan<>'])
        )->assertSessionHasErrors('first_name');
    }
    ...
    function the_last_name_may_only_contain_letters()
    {
        $this->post(
            route('register'),
            $this->userValidData(['last_name' => 'Garcia2'])
        )->assertSessionHasErrors('last_name');

        $this->post(
            route('register'),
            $this->userValidData(['last_name' => 'Garcia<>'])
        )->assertSessionHasErrors('last_name');
    }
